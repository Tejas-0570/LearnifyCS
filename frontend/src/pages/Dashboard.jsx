...
const CodeParticles3D = () => {
  ...
  useEffect(() => {
    ...
    // particles on elliptical orbits
    const particles = Array.from({ length: 38 }, (_, i) => ({
      ...
      yFlatten: 0.32 + (i % 3) * 0.06,
      yOffset: (i % 7 - 3) * 12,
    }));

    // floating symbols with independent bobbing
    const symbols = [
    ...
    // 3D floating mini-nodes (small spheres in 3D space)
    const nodes3d = Array.from({ length: 12 }, (_, i) => ({
      x: (Math.random() - 0.5) * 260,
      y: (Math.random() - 0.5) * 180,
      z: (Math.random() - 0.5) * 120,
      r: 3 + Math.random() * 3,
      rotX: Math.random() * 0.004,
    ...
    function drawFrame() {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < i + 4 && j < particles.length; j++) {
          ...
          const dist = Math.hypot(pjx - pix, pjy - piy);
          if (dist < 60) {
            ctx.beginPath();
            ctx.moveTo(pix, piy);
            ctx.lineTo(pjx, pjy);
      ...
      symbols.forEach(s => {
        const sy = cy + s.baseY + Math.sin(t * s.bobSpeed + s.phase) * s.bobAmp;
        const sx = cx + s.baseX;

        // subtle glow behind big symbols
        if (s.size > 30) {
        ...
        ctx.globalAlpha = s.size > 30 ? 0.88 : 0.65;
...
const modules = [
  {
    ...
    short: 'DSA',
    desc: 'Visualize arrays, trees, graphs, sorting and more through step-by-step interactive animations.',
    path: '/dsa',
    active: true,
    progress: 82,
    topics: 10,
    ...
    badgeColor: '#1d4ed8',
    Icon: IconDSA,
    highlights: ['BFS / DFS / Dijkstra', '6 Sorting Algorithms', 'Hash Table Visualizer'],
  },
  {
    id: 'os',
    label: 'Operating Systems',
    short: 'OS',
    desc: 'Explore CPU scheduling, memory management, deadlock detection and process synchronization.',
    path: '/os',
    active: false,
    progress: 0,
    topics: 7,
    ...
    badgeColor: '#6d28d9',
    Icon: IconOS,
    highlights: ['CPU Scheduling', "Banker's Algorithm", 'Disk Scheduling'],
  },
  {
    id: 'dbms',
    label: 'Database Management',
    short: 'DBMS',
    desc: 'Understand ER models, relational algebra, SQL queries, ACID properties and transactions.',
    path: '/dbms',
    active: false,
    progress: 0,
    topics: 6,
    ...
    badgeColor: '#0e7490',
    Icon: IconDBMS,
    highlights: ['ER Diagrams', 'SQL Queries', 'ACID Properties'],
  },
  {
    id: 'cn',
    label: 'Computer Networks',
    short: 'CN',
    desc: 'Learn OSI model, TCP/IP, routing algorithms, error detection and wireless networks.',
    path: '/cn',
    active: false,
    progress: 0,
    topics: 6,
    ...
    badgeColor: '#047857',
    Icon: IconCN,
    highlights: ['OSI / TCP-IP Model', 'Routing Algorithms', 'Error Detection'],
  },
];
...
const Dashboard = () => {
  ...
  const userName = 'Tejas'; // replace with auth context later
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f8fafc' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <Box sx={{ flex: '0 0 55%', py: 5 }}>
            ...
            <Typography
              variant="h4" fontWeight={800} color="#0f172a" mb={1.5}
            ...
            <Box display="flex" gap={2} flexWrap="wrap">
              {stats.map((s) => (
                <Box key={s.label} sx={{
                  ...
                  <Typography variant="h5" fontWeight={800} color="#2563eb" sx={{ letterSpacing: '-0.5px' }}>
                    {s.value}
                  </Typography>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  borderRadius: 3,
                  border: '1.5px solid',
                  borderColor: isHov && mod.active ? mod.color : mod.borderColor,
                  overflow: 'hidden',
                  cursor: mod.active ? 'pointer' : 'default',
                  opacity: mod.active ? 1 : 0.72,
                  transition: 'all 0.22s ease',
                  transform: isHov && mod.active ? 'translateY(-4px)' : 'none',
                  boxShadow: isHov && mod.active
              ...
              >
                {/* Strip */}
                <Box sx={{
                  bgcolor: mod.lightBg, px: 2.5, pt: 2.5, pb: 2,
                  ...
                  <Box sx={{ bgcolor: mod.badgeBg, px: 1.2, py: 0.5, borderRadius: 5 }}>
                    <Typography sx={{
                      fontSize: 10, fontWeight: 700, color: mod.badgeColor,
                ...
                <Box sx={{ px: 2.5, pt: 1.2, pb: 2.5 }}>
                  ...
                  <Box display="flex" flexWrap="wrap" gap={0.8} mb={2}>
                    {mod.highlights.map((h) => (
                      <Box key={h} sx={{
                        ...
                        <Typography sx={{ fontSize: 11, fontWeight: 600, color: mod.color }}>
                          {h}
                        </Typography>
                      </Box>
                    ))}
                  ...
                  {mod.active && (
                    <Box mb={2}>
                      <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.8}>
                        ...
                        <Typography variant="caption" fontWeight={700} sx={{ color: mod.color }}>
                          {mod.progress}%
                        </Typography>
                      </Box>
                      <LinearProgress
                        ...
                        sx={{
                          ...
                          '& .MuiLinearProgress-bar': { bgcolor: mod.color, borderRadius: 99 },
                        }}
                      />
                    </Box>
                  )}
                  ...
                  <Button
                    ...
                    sx={{
                      ...
                      borderRadius: 2.5, py: 1.1, boxShadow: 'none',
                      bgcolor: mod.active ? mod.color : '#9ca3af',
                      '&:hover': { bgcolor: mod.active ? mod.color : '#9ca3af', opacity: 0.88, boxShadow: 'none' },
          ...
          {quickTopics.map((t) => (
            <Box
              ...
              sx={{
                px: 2.2, py: 1,
                border: '1.5px solid',
                borderColor: hoveredTopic === t.label ? t.color : `${t.color}44`,
                borderRadius: 99,
                bgcolor: hoveredTopic === t.label ? t.color : '#ffffff',
                cursor: 'pointer',
            ...
            >
              <Typography
                ...
                sx={{ color: hoveredTopic === t.label ? '#ffffff' : 'text.primary' }}
              >
                {t.label}
              </Typography>
            </Box>
      ...
      <Box sx={{
        ...
        <Typography variant="caption" color="text.secondary">
          LearnifyCS — Built for learners, by learners
        </Typography>
      </Box>
